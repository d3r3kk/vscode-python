...
function MochaVstsReporter(runner, options) {
  ...
  this._succeededTestsCount = 0;
  this._skippedTestCount = 0;
  ...
  this._runner.on('pass', function(test) {
    ...
    if (this._currentTestSuiteStatus === 'Unknown') {
      this._currentTestSuiteStatus = 'Succeeded';
    }
    console.log(  // eslint-disable-line no-console
      '##vso[task.logdetail id=%s;state=InProgress]PASS: %s (%dms).',
      this._currentTestSuiteTimeline,
      test.title,
      test.duration);
    this._succeededTestsCount += 1;
  }.bind(this));

  this._runner.on('fail', function(test, err) {
    lastSuite().push(this.getTestcaseData(test, err));
    this._currentTestSuiteStatus = 'Failed';
    console.log(  // eslint-disable-line no-console
      '##vso[task.logdetail id=%s;state=InProgress]FAIL: %s (%dms).',
      this._currentTestSuiteTimeline,
      test.title,
      test.duration);

    this._failedTestsCount += 1;
  }.bind(this));
  ...
  if (this._options.includePending) {
    this._runner.on('pending', function(test) {
      ...
      console.log(  // eslint-disable-line no-console
        '##vso[task.logdetail id=%s;state=InProgress]SKIP: %s (%dms).',
        this._currentTestSuiteTimeline,
        test.title,
        test.duration);

      this._skippedTestCount += 1;
    }.bind(this));
  }

  this._runner.on('end', function() {
    overallStatus = 'Unknown';
    if (this._failedTestsCount > 0) {
      overallStatus = 'Failed';
    }
    else if (this._succeededTestsCount > 0) {
      if (this._skippedTestCount > 0) {
        overallStatus = 'SucceededWithIssues';
      }
      else {
 ...
 */
MochaVstsReporter.prototype.getTestsuiteData = function(suite) {
  var testSuite = {
    testsuite: [
      {
        _attr: {
 ...
 * @returns {object}
 */
MochaVstsReporter.prototype.getTestcaseData = function(test, err) {
  var config = {
    testcase: [{
      _attr: {
 ...
 */
MochaVstsReporter.prototype.removeInvalidCharacters = function(input){
 ...
 */
MochaVstsReporter.prototype.flush = function(testsuites){
 ...
 * @returns {string}
 */
MochaVstsReporter.prototype.getXml = function(testsuites) {
 ...
 */
MochaVstsReporter.prototype.writeXmlToDisk = function(xml, filePath){
  if (filePath) {
